...
The research is organized around three research questions, each answered by a dedicated set of computational experiments:

\begin{itemize}
    \item RQ1: Which of the residual super-resolution architectures, trained with a pixel-wise loss, delivers the lowest point-wise reconstruction error on the three-channel state fields of the reservoir?
    ...
    \item RQ3: Does supplying the super-resolution generator with a vector of dynamic simulation parameters improve reconstruction quality compared to an unconditional baseline?
\end{itemize}

The following hypotheses are formulated.

\textbf{Hypothesis~1.} Among residual super-resolution architectures trained with a pixel-wise loss, a deeper architecture yields a lower point-wise reconstruction error than lighter architectures.

\begin{itemize}
    \item Independent variable: residual super-resolution architecture.
...
\textbf{Hypothesis~2.} Fine-tuning residual super-resolution architectures within an adversarial framework increases the structural and spectral fidelity of the reconstruction and simultaneously decreases its point-wise accuracy.
...
\textbf{Hypothesis~3.} Supplying the super-resolution generator with a vector of dynamic simulation parameters, in addition to the coarse-grid input state, reduces the reconstruction error compared to an unconditional baseline.

\begin{itemize}
    \item Independent variable: presence of a vector of dynamic conditioning parameters at the generator input.
    \item Dependent variable: reconstruction error measured by point-wise and structural-spectral metrics.
    \item Population: spatial distributions of multiphase flow state variables within the studied computational domain.
    \item Relation: the same coarse-grid state may correspond to several distinct fine-grid states under different reservoir operating regimes, and the vector of dynamic parameters removes this ambiguity.
\end{itemize}

To achieve the goal and test the hypotheses, the following objectives are defined:

\begin{enumerate}
    \item Conduct a domain analysis of the limitations of coarse-grid numerical modeling in fractured-porous media and review existing neural network approaches to spatial super-resolution.

    \item Implement and train three residual super-resolution architectures with a pixel-wise loss to establish point-wise accuracy baselines.
    ...
    \item Extend the baseline architecture with a feature-wise linear modulation mechanism conditioned on a vector of dynamic simulation parameters and compare it against the unconditional baseline.
    ...
    \item Develop a prototype desktop application based on the PySide6 framework with integration to the simulator through a gRPC interface for the practical demonstration of the selected model and for side-by-side comparison with classical interpolation and reference fine-grid solutions.
\end{enumerate}

The practical significance of this work lies in reducing the computational cost of high-fidelity reservoir forecasting by partially replacing fine-grid simulation runs with neural super-resolution applied to coarse-grid solutions. This enables larger ensemble sizes in uncertainty quantification workflows without a proportional increase in high-performance computing requirements, making probabilistic decision-making more accessible in field development planning. The accompanying desktop application integrates with the existing simulation infrastructure through a gRPC interface and allows engineers to compare classical interpolation, neural super-resolution, and reference fine-grid solutions in a unified visual environment.
