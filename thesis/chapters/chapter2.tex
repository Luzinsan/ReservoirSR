...
The regression-to-the-mean effect described in Section~\ref{subsec:dl_foundations} can be made precise. For a generator $G_\theta$ trained by minimizing the expected squared error between $G_\theta(X)$ and the high-resolution target $Y$, classical estimation theory shows that the optimum is the conditional expectation:
\begin{equation}
    G_\theta^*(X) = \mathbb{E}[Y \mid X].
\end{equation}
When several plausible high-resolution outputs are consistent with a given low-resolution input, this optimum is a deterministic spatial average and cannot reproduce the sharp features of any individual realization. Generative adversarial networks were introduced to optimize a different objective, one that matches the distribution of outputs to the distribution of high-resolution targets rather than the conditional mean.
...
The adversarial framework improves the recovery of high-frequency content but is also known to produce sample-level artifacts that the discriminator alone does not penalize, since the adversarial objective constrains the distribution of outputs rather than the fidelity of any individual reconstruction. Additional constraints are therefore needed to keep the generated samples close to plausible reconstructions, either through physics-aware regularization terms acting on the generator output, examined in Section~\ref{sec:physics_sr}, or through auxiliary conditioning, examined in Section~\ref{subsec:conditioning}.

\subsection{Conditioning Mechanisms for Multi-task Generators}
\label{subsec:conditioning}

Conditioning mechanisms extend convolutional super-resolution networks with auxiliary scalar information that modulates the network's activations. They are needed when the output depends on context that is not encoded in the spatial input.

Several strategies for injecting scalar context into a convolutional generator have appeared in the literature. The simplest is direct concatenation, in which the conditioning vector is tiled along the spatial dimensions and appended as additional input channels; this inflates the input dimensionality of the first convolutional layer and offers no interaction with deeper layers. Dumoulin et al.~\cite{dumoulin2017conditional} introduced Conditional Batch Normalization, in which the global affine parameters of a Batch Normalization layer are replaced by per-style parameters predicted from the conditioning vector. Huang and Belongie~\cite{huang2017adain} extended this idea to Adaptive Instance Normalization (AdaIN), which aligns the channel-wise mean and variance of the content features with those of the style features. Both mechanisms are conditioning-by-normalization: they require a normalization layer in the backbone to apply the predicted scale and shift.

Perez et al.~\cite{perez2018film} introduced Feature-wise Linear Modulation (FiLM), a conditioning mechanism that does not require normalization layers. FiLM applies an affine transformation to the activations of an intermediate convolutional layer, with the scale and shift parameters produced by an auxiliary network from a conditioning vector. Formally, for a feature map $F_c$ of channel $c$ and conditioning vector $\mathbf{z}$, the modulated activation is
\begin{equation}
    \widetilde{F}_c = \gamma_c(\mathbf{z}) F_c + \beta_c(\mathbf{z}),
\end{equation}
where the functions $\gamma_c(\cdot)$ and $\beta_c(\cdot)$ are jointly learned with the main network. The same modulation is applied uniformly across spatial positions within each channel, which keeps the parameter overhead low and lets the conditioning vector influence every block of the backbone. Because FiLM does not rely on normalization statistics, it is compatible with architectures that operate without Batch Normalization. Perez et al. demonstrated FiLM on visual reasoning tasks, and the mechanism has since been adopted in multi-task super-resolution, image-to-image translation, and conditional generative models.
...
The application of super-resolution to subsurface hydrodynamics differs from standard computer vision in the nature of the input data. Standard super-resolution assumes that the low-resolution input is obtained by applying a deterministic blurring and downsampling operator to a high-resolution original. In computational physics, the low-resolution input is a numerical solution of the governing equations on a coarse mesh; it contains numerical diffusion and omits subgrid-scale phenomena. The mapping that the network must learn therefore recovers physical information lost during coarse-grid simulation rather than inverting a known blurring kernel.
