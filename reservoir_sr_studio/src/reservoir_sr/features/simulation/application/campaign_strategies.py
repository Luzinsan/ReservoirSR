...
class CampaignSamplingStrategy(Protocol):
    ...
    def build_cases(self, request: SimulationCampaignRequest) -> list[SimulationCampaignCase]: ...
...
class LatinHypercubeSamplingStrategy:
    ...
    def build_cases(self, request: SimulationCampaignRequest) -> list[SimulationCampaignCase]:
        ...
        for idx, parameter_range in enumerate(ranges):
            ...
            values = _map_unit_to_range(perm, parameter_range)
            dims[:, idx] = values
        ...
        for case_index in range(n):
            ...
            cfg = replace(
                cfg,
                nx=request.lr_nx,
                tu_seconds=request.fixed_tu_seconds,
                epsp=request.fixed_epsp,
            )
